...
const GoogleSheets = ({
  ...
  const clearStaleTasks = async () => {
    ...
    try {
      await axios.post(
        `${apiBaseUrl}/user/clear-stale-tasks`,
        {},
        {
  ...
  const addSpreadsheet = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { ...form, gid: Number.parseInt(form.gid), intervalHours: Number.parseInt(form.intervalHours) },
        { headers: { Authorization: `Bearer ${token}` } },
      )
  ...
  const runAnalysis = async (spreadsheetId) => {
    ...
    try {
      const response = await axios.post(
        `${apiBaseUrl}/${projectId}/spreadsheets/${spreadsheetId}/run`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const cancelAnalysis = async (spreadsheetId) => {
    ...
    try {
      const response = await axios.post(
        `${apiBaseUrl}/${projectId}/spreadsheets/${spreadsheetId}/cancel`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  return (
    <motion.div className="max-w-full mx-auto" initial="hidden" animate="visible" variants={fadeInUp}>
      <button
                ...
                {(isRunning || progress.status === "pending") && isProjectAnalyzing && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="text-gray-600 text-sm">
                      ...
                      <p>
                        Processed: {progress.processedLinks} / {progress.totalLinks} links
                      </p>
